...
class ControllerCatalogProduct extends Controller {
	...
	protected function getList() {
		...
		if (isset($this->request->get['filter_cost'])) {
			$filter_cost = html_entity_decode(
				$this->request->get['filter_cost'],
				ENT_QUOTES,
				'UTF-8'
			);
		}
		...
		if (isset($this->request->get['filter_profit'])) {
			$filter_profit = html_entity_decode(
				$this->request->get['filter_profit'],
				ENT_QUOTES,
				'UTF-8'
			);
		}
		...
		if ($filter_model !== null && $filter_model !== '') {
			$url .= '&filter_model=' . urlencode($filter_model);
		}

		if ($filter_category) {
			$url .= '&filter_category=' . (int)$filter_category;
		}



		if ($filter_price !== null && $filter_price !== '') {
			$url .= '&filter_price=' . urlencode($filter_price);
		}
		...
		if ($filter_quantity !== null && $filter_quantity !== '') {
		...
		foreach ($categories as $category) {
			$this->data['filter_categories'][] = array(
				...
				'name'        => $this->model_catalog_category->getCategoryPath($category['category_id'])
			);
		}
		...
		$this->data['column_cost'] = 'Cost';
		$this->data['column_profit'] = 'Profit';
		$this->data['column_category'] = 'Category';
	...
	protected function getForm() {
		...
		} elseif (!empty($product_info) && isset($product_info['cost'])) {
