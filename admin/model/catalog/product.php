...
class ModelCatalogProduct extends Model {
	...
	foreach ($data['product_category'] as $category_id) {
		...
		$this->getCategoryParents($category_id, $all_categories);
	}
	...
	if (isset($data['product_profiles'])) {
		foreach ($data['product_profiles'] as $profile) {
			$this->db->query("
				INSERT INTO " . DB_PREFIX . "product_profile 
				SET product_id = '" . (int)$product_id . "', 
	...
	private function getCategoryParents($category_id, &$categories = array()) {
	$query = $this->db->query("
		SELECT parent_id 
	...
	if ($query->num_rows && $query->row['parent_id'] > 0) {
		...
		if (!in_array($parent_id, $categories)) {
			...
			$this->getCategoryParents($parent_id, $categories);
		}
	}
}
	...
	$this->db->query("DELETE FROM " . DB_PREFIX . "product_layout WHERE product_id = '" . (int)$product_id . "'");
    ...
    // Category filter JOIN (ONLY ONCE)
    if (!empty($data['filter_category'])) {
    $sql .= " LEFT JOIN " . DB_PREFIX . "product_to_category pc
              ON (p.product_id = pc.product_id)";

    $sql .= " AND pc.category_id IN (
        SELECT category_id
        FROM " . DB_PREFIX . "category_path
        WHERE path_id = '" . (int)$data['filter_category'] . "'
    )";
}

    $sql .= " WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";

    // Category filter (with full tree using category_path)
    if (!empty($data['filter_category'])) {
        $sql .= " AND pc.category_id IN (
                    SELECT category_id
                    FROM " . DB_PREFIX . "category_path
    ...
    // Price filter (supports inequalities)
    if (!empty($data['filter_price'])) {
        $value = str_replace(' ', '', trim($data['filter_price']));

        if (preg_match('/^(>=|<=|!=|>|<)(\d+(\.\d+)?)$/', $value, $m)) {
            $sql .= " AND p.price {$m[1]} " . (float)$m[2];
        } elseif (is_numeric($value)) {
            $sql .= " AND p.price = " . (float)$value;
        }
    }

    // Cost filter (supports inequalities)
    if (!empty($data['filter_cost'])) {
        $value = html_entity_decode($data['filter_cost'], ENT_QUOTES, 'UTF-8');
        $value = str_replace(' ', '', trim($value));

        if (preg_match('/^(>=|<=|!=|>|<)(\d+(\.\d+)?)$/', $value, $m)) {
            $sql .= " AND p.cost {$m[1]} " . (float)$m[2];
        } elseif (is_numeric($value)) {
            $sql .= " AND p.cost = " . (float)$value;
        }
    }

    // Profit filter (supports inequalities)
    if (!empty($data['filter_profit'])) {
        $value = html_entity_decode($data['filter_profit'], ENT_QUOTES, 'UTF-8');
        $value = str_replace(' ', '', trim($value));

        if (preg_match('/^(>=|<=|!=|>|<)(\d+(\.\d+)?)$/', $value, $m)) {
            $sql .= " AND (p.price - p.cost) {$m[1]} " . (float)$m[2];
        } elseif (is_numeric($value)) {
            $sql .= " AND (p.price - p.cost) = " . (float)$value;
        }
    }
    ...
    if (isset($data['sort'])) {
        ...
        } elseif (in_array($data['sort'], $sort_data)) {
    ...
    // Category filter using full path (category_path)
    if (!empty($data['filter_category'])) {
        $sql .= " AND pc.category_id IN (
                    SELECT category_id
                    FROM " . DB_PREFIX . "category_path
	...
	while ($category_id) {
		...
		array_unshift($path, $query->row['name']);
