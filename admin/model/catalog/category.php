...
class ModelCatalogCategory extends Model {
    ...
    if (!empty($data['filter_product_count'])) {
        $value = str_replace(' ', '', trim($data['filter_product_count']));

        if (preg_match('/^(>=|<=|!=|>|<)(\d+)$/', $value, $m)) {
            $sql .= " HAVING product_count {$m[1]} " . (int)$m[2];
        } elseif (is_numeric($value)) {
            $sql .= " HAVING product_count = " . (int)$value;
        }
    }
    ...
    if (!empty($data['sort'])) {
        if ($data['sort'] == 'product_count') {
            $sort = 'product_count';
        } elseif ($data['sort'] == 'sort_order') {
            $sort = 'c.sort_order';
        }
    }
    ...
    if (!empty($data['filter_product_count'])) {
        ...
        if (preg_match('/^(>=|<=|!=|>|<)(\d+)$/', $value, $m)) {
            $sql .= " HAVING COUNT(DISTINCT p2c.product_id) {$m[1]} " . (int)$m[2];
        } elseif (is_numeric($value)) {
            $sql .= " HAVING COUNT(DISTINCT p2c.product_id) = " . (int)$value;
        }
    }

    $query = $this->db->query("SELECT COUNT(*) AS total FROM (" . $sql . ") t");
    ...
    foreach ($query->rows as $row) {
        $categories = array_merge(
            $categories,
            $this->getCategoryChildren($row['category_id'])
        );
    }
